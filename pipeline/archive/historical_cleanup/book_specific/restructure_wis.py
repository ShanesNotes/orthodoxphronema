#!/usr/bin/env python3
"""
Restructure WIS.md — reassign verse anchors via Jaccard matching against Brenton.

The extractor only detected chapters 1-10; chapters 11-19 were all mis-labelled
as WIS.10:N.  This script:
  ...
  6. Writes a clean WIS.md with correct anchors and chapter headers.
...
WIS_MD   = REPO / "staging/validated/OT/WIS.md"
BRENTON  = REPO / "staging/reference/brenton/WIS.json"
...
def parse_wis_md(path: Path):
    ...
    # - WIS.1:1 thru WIS.1:13 appear to be article/outline content
    # - First real verse content matches WIS.1:1 Brenton "Love righteousness..."
    ...
    ARTICLE_NOISE_ANCHORS = {
        'WIS.1:1', 'WIS.1:10', 'WIS.1:11', 'WIS.1:12', 'WIS.1:13',
    }
    ...
    for i, line in enumerate(lines[fm_end+1:], start=fm_end+1):
        ...
        if s.startswith('WIS.'):
            ...
            if anchor in ARTICLE_NOISE_ANCHORS:
                # Skip article/intro noise
                pending_headers = []
                continue
...
def main():
    print("=== WIS restructure ===")

    # Load data
    fm_lines, verse_entries = parse_wis_md(WIS_MD)
    ...
    for b in reg['books']:
        if b.get('code') == 'WIS':
            ...
            break
    assert cvc is not None, "WIS not found in registry"
    ...
    # Write back
    WIS_MD.write_text(output, encoding='utf-8')
    print(f"\nWrote {len(final_verses)} verses to {WIS_MD}")
    ...
    if low_confidence:
        ...
        for i, (ch, v, score) in low_confidence:
            print(f"  WIS.{ch}:{v} score={score:.3f}: {verse_entries[i]['text'][:80]}")
