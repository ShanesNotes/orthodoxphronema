...
OT_CANON_DIR = REPO_ROOT / "canon" / "OT"
REPORT_PATH = REPO_ROOT / "reports" / "ot_structural_census.json"
MEMO_PATH = REPO_ROOT / "memos" / "93_ot_structural_census.md"
...
def run_census():
    books = []
    ot_files = sorted(list(OT_CANON_DIR.glob("*.md")))
    ...
    memo_content = template.replace("<insert title>", "OT Structural Census (Canon)")
    memo_content = memo_content.replace("<insert objective>", "Systematic structural reconnaissance across all promoted OT canon books.")
...
"""
    ...
    findings += f"""
### Clean Books (Zero Issues)
- {", ".join(clean_books) if clean_books else "None"}

### Detailed Stats
Stored in `reports/ot_structural_census.json`.
"""

    memo_content = memo_content.replace("<insert findings>", findings)
    memo_content = memo_content.replace("<insert risk analysis>", "The OT canon shows significantly higher structural stability than the NT staging. Most issues are limited to expected V7-class verse gaps (Minor) or legacy note markers (Unstable).")
    
    handshake = f"""
- `Files changed`: `reports/ot_structural_census.json`, `memos/93_ot_structural_census.md`
- `Verification run`: Census validation script run against {len(books)} books.
- `Artifacts refreshed`: Census report and memo.
- `Remaining known drift`: Localized V4/V7 gaps and legacy note-marker traces.
- `Next owner`: Ark (Canon Stability Audit)
"""
